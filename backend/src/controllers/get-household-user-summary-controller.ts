...
import type { GetHouseholdUserSummaryService } from '../services/get-household-user-summary-service.js';
import { transactionDateSchema } from './transaction-schemas.js';

const householdIdSchema = z.uuid();
const userSummaryQuerySchema = z.strictObject({
  startDate: transactionDateSchema.optional(),
  endDate: transactionDateSchema.optional(),
});

export function getHouseholdUserSummaryController(
  service: GetHouseholdUserSummaryService,
): RequestHandler {
  return async (request, response, next) => {
    ...
    const parsedQuery = userSummaryQuerySchema.safeParse(request.query);
