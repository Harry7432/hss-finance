...
describe('app router', () => {
  ...
  it('redirects an unauthenticated user from the categories route to login', async () => {
    fetchMock.mockResolvedValue(
      new Response(
        JSON.stringify({ error: { code: 'UNAUTHORIZED', message: 'Authentication required' } }),
        { headers: { 'Content-Type': 'application/json' }, status: 401 },
    ...
    renderProtectedRoute('/app/categories');

    expect(await screen.findByRole('heading', { name: 'Entrar' })).toBeInTheDocument();
  });

  it('renders the categories route for an authenticated user', async () => {
    fetchMock.mockResolvedValue(
      new Response(
        JSON.stringify({
          data: {
  ...
  it('redirects an unauthenticated user from the family route to login', async () => {
    fetchMock.mockResolvedValue(
      new Response(
        JSON.stringify({ error: { code: 'UNAUTHORIZED', message: 'Authentication required' } }),
        { headers: { 'Content-Type': 'application/json' }, status: 401 },
    ...
    renderProtectedRoute('/app/family');

    expect(await screen.findByRole('heading', { name: 'Entrar' })).toBeInTheDocument();
  });

  it('renders the family route for an authenticated user', async () => {
    fetchMock.mockResolvedValue(
      new Response(
        JSON.stringify({
          data: {
    ...
    expect(await screen.findByRole('heading', { name: 'Família' })).toBeInTheDocument();
  });
